...
for c in attrs_collect(0):
    attr_dict[c] = len(attr_dict)
...
class Meta512Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        l = self.label["labels"][self.label['Img'] == self.all_data_id[self.ids[index]].rsplit('/')[-1]].values[0]
        label = np.array([0] * 33)
        label[attr_dict[l]] = 1
