...
from vision_model import densenet, efficientnet, densenet_201, Resnext, Resnest


"""
Test for only one model used.
...
def network_map(net):
    network_mapping = {
        ...
        'meta_resnext': Resnext(use_meta=True),
        'meta_resnest': Resnest(use_meta=True),
    }
    ...
    with open(f'result/{args.log_name}_prob.csv', 'w') as f:
     ...
     'redbeans', 'roseapple', 'sesbania', 'soybeans', 'sunhemp', 'sweetpotato', 'taro', 'tea', 'waterbamboo'])
        for i in range(len(pred)):
            tmp = [id_t[i]] + [x.item() for x in pred[i]]
            writer.writerow(tmp)
    with open(f'result/{args.log_name}.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['filename', 'label'])
        for i in range(len(pred)):
            tmp = [id_t[i], attrs_default[pred_max[i].item()]]
            writer.writerow(tmp)
